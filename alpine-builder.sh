...
alpine=$(buildah from alpine:3.17) || die 1 "Could not get alpine base image"
...
wget -O- "https://alpha.de.repo.voidlinux.org/static/xbps-static-latest.$(uname -m)-musl.tar.xz" | tar Jx -C "$alpine_mount"
XBPS_ARCH=$ARCH
export XBPS_ARCH
echo $REPO_MUSL
